...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        # Use all data to show the overall success vs. failure count
        success_counts = spacex_df['class'].value_counts().reindex([0, 1], fill_value=0)
        fig = px.pie(
...
def update_scatter_chart(selected_site, payload_range):
    # Filter dataframe based on the selected site
    if selected_site == 'ALL':
        filtered_df = spacex_df
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == selected_site]
    
    # Further filter dataframe based on the payload range
    filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= payload_range[0]) &
                              (filtered_df['Payload Mass (kg)'] <= payload_range[1])]
    
    # Create a scatter plot with Payload Mass vs. Launch Outcome
    fig = px.scatter(
        filtered_df,
        x='Payload Mass (kg)',
        y='class',
        ...
        category_orders={'class': [0, 1]}  # Ensure consistent color for Success and Failure
    )
    return fig
